...
import { getVerificationReminderEmailTemplate } from '../src/lib/email/templates';
...
async function main() {
    console.log(`🚀 Starting Verification Reminder Campaign for ${MERCHANT_SLUG} (Aliyun SMTP)`);
    ...
    const mContent = merchant.content || {};
    const merchantAddress = mContent.address;
    const merchantPhone = mContent.phone;
    ...
    for (const recipient of recipients) {
        try {
            await transporter.sendMail({
                ...
                to: recipient.email,
                subject: `We're waiting for you at Honoo Ramen Bar! 🍜`,
                html: getVerificationReminderEmailTemplate({
                    name: recipient.name || '',
